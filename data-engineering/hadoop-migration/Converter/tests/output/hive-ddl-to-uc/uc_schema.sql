...
-- ----------------------------------------------------------------------------
USE CATALOG main;
USE SCHEMA hm_retail_analytics;

-- ----------------------------------------------------------------------------
-- Database → Schema
-- Hive:  CREATE DATABASE ... LOCATION 'hdfs://...'
-- UC:    CREATE SCHEMA (LOCATION removed — UC manages storage for managed schemas)
--        DBPROPERTIES → SCHEMA PROPERTIES (not supported in UC; documented here)
-- ----------------------------------------------------------------------------
CREATE SCHEMA IF NOT EXISTS main.hm_retail_analytics
COMMENT 'Retail analytics data warehouse (migrated from Hive)'
-- NOTE: Original DBPROPERTIES were: owner=data_engineering, environment=production
-- These can be managed via Unity Catalog tags or schema-level TBLPROPERTIES.
;
...
--   - JsonSerDe → USING DELTA (preferred; load JSON data via Auto Loader / COPY INTO)
--   - PARTITIONED BY columns merged into schema definition
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS main.hm_retail_analytics.raw_clickstream (
    session_id      STRING,
    user_id         STRING,
    event_type      STRING,
    page_url        STRING,
...
-- COPY INTO main.hm_retail_analytics.raw_clickstream
...
--   - CLUSTERED BY ... INTO 16 BUCKETS → CLUSTER BY (Liquid Clustering)
--   - ORC-specific props removed (orc.compress, orc.create.index)
--   - transactional prop removed (Delta is ACID-native)
--   - Delta optimization properties added
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS main.hm_retail_analytics.dim_customers (
    customer_id     BIGINT,
    customer_key    STRING,
    first_name      STRING,
    last_name       STRING,
...
--   - Delta optimization properties added
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS main.hm_retail_analytics.fact_orders (
    order_id        BIGINT,
    customer_id     BIGINT,
    product_id      BIGINT,
    order_date      DATE,
...
--   - Delta optimization properties added
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS main.hm_retail_analytics.product_catalog (
    product_id      BIGINT,
    sku             STRING,
    name            STRING,
    description     STRING,
...
-- Changes:
--   - 3-level namespace applied to all table references in the query body
-- ----------------------------------------------------------------------------
CREATE VIEW IF NOT EXISTS main.hm_retail_analytics.vw_active_customers AS
SELECT
    c.customer_id,
    c.first_name,
    c.last_name,
...
FROM main.hm_retail_analytics.dim_customers c
LEFT JOIN main.hm_retail_analytics.fact_orders o ON c.customer_id = o.customer_id
...
-- Data loading (run after table creation):
-- COPY INTO main.hm_retail_analytics.raw_logs
...
--   ALTER TABLE main.hm_retail_analytics.dim_customers CLUSTER BY (customer_id, email);
-- ----------------------------------------------------------------------------
...
--   1. USE retail_analytics          → USE CATALOG main; USE SCHEMA retail_analytics;
--   2. CREATE DATABASE               → CREATE SCHEMA (LOCATION removed)
--   3. CREATE EXTERNAL TABLE         → CREATE TABLE USING DELTA (2 tables)
--      raw_clickstream                  JsonSerDe removed; load via COPY INTO
--      raw_logs                         Custom I/O formats removed; load via COPY INTO
--   4. CREATE TABLE                  → CREATE TABLE USING DELTA (3 tables)
--      dim_customers                    Bucketing → Liquid Clustering
--      fact_orders                      Partition cols merged into schema
--      product_catalog                  Complex types preserved
--   5. CREATE VIEW                   → CREATE VIEW with 3-level namespace refs
--   6. ALTER TABLE ADD PARTITION     → Removed (Delta auto-manages partitions)
...
--   [!] Original DBPROPERTIES (owner, environment) should be recreated
--       using Unity Catalog tags if needed
-- =============================================================================
