...
-- Target: catalog=aa_catalog, schema=bny
...
-- ----------------------------------------------------------------------------
CREATE SCHEMA IF NOT EXISTS aa_catalog.bny
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS aa_catalog.bny.raw_clickstream (
    session_id      STRING,
    user_id         STRING,
    event_type      STRING,
    page_url        STRING,
...
-- COPY INTO aa_catalog.bny.raw_clickstream
...
--   - Delta optimization properties added
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS aa_catalog.bny.dim_customers (
    customer_id     BIGINT,
    customer_key    STRING,
    first_name      STRING,
    last_name       STRING,
...
--   - Delta optimization properties added
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS aa_catalog.bny.fact_orders (
    order_id        BIGINT,
    customer_id     BIGINT,
    product_id      BIGINT,
    order_date      DATE,
...
--   - Delta optimization properties added
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS aa_catalog.bny.product_catalog (
    product_id      BIGINT,
    sku             STRING,
    name            STRING,
    description     STRING,
...
-- Changes:
--   - 3-level namespace applied to all table references (aa_catalog.bny.*)
-- ----------------------------------------------------------------------------
CREATE VIEW IF NOT EXISTS aa_catalog.bny.vw_active_customers AS
SELECT
    c.customer_id,
    c.first_name,
    c.last_name,
...
FROM aa_catalog.bny.dim_customers c
LEFT JOIN aa_catalog.bny.fact_orders o ON c.customer_id = o.customer_id
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS aa_catalog.bny.raw_logs (
    log_line    STRING,
...
-- Data loading (run after table creation):
-- COPY INTO aa_catalog.bny.raw_logs
...
--   ALTER TABLE aa_catalog.bny.dim_customers CLUSTER BY (customer_id, email);
-- ----------------------------------------------------------------------------

-- =============================================================================
-- MIGRATION SUMMARY
-- =============================================================================
-- Target namespace: aa_catalog.bny
--
-- Statements converted:
--   1. USE retail_analytics          → USE CATALOG aa_catalog; USE SCHEMA bny;
--   2. CREATE DATABASE               → CREATE SCHEMA aa_catalog.bny (LOCATION removed)
