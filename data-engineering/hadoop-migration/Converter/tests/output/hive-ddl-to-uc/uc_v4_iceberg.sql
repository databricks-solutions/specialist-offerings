-- =============================================================================
-- Unity Catalog DDL — Converted from Hive DDL (Iceberg format)
-- Source: Converter/tests/input/hive-ddl-to-uc/hive_schema.hql
-- Target: catalog=aa_catalog, schema=retail_analytics
-- Format: ICEBERG (user-specified override; default is DELTA)
-- Table suffix: _ice
...
--   - Note: Views are not format-specific; no _ice suffix applied to the view name
-- ----------------------------------------------------------------------------
...
--      product_catalog_ice              Complex types preserved
...
--
-- Key changes:
--   - All tables use USING ICEBERG instead of USING DELTA
--   - All table names suffixed with _ice
--   - All PARTITIONED BY and CLUSTERED BY → CLUSTER BY (Liquid Clustering)
--   - Delta-specific TBLPROPERTIES (autoOptimize) not included (Iceberg format)
--
...
--   [!] Iceberg tables in UC: Ensure workspace has UniForm or Iceberg support enabled
-- =============================================================================
